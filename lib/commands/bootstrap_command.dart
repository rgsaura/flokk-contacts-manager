...
class BootstrapCommand extends AbstractCommand {
  ...
  Future<bool> execute() async {
    ...
    // Disabled hardcoded token to prevent invalid_grant errors
    bool sideStepLoginFlow = false;
    ...
    if (sideStepLoginFlow) {
      ...
      authModel.googleRefreshToken = "PLACEHOLDER_REFRESH_TOKEN"; // Remove actual token for security
    ...
    if (authModel.hasAuthKey && !sideStepLoginFlow) {
      ...
      bool authSuccess;
      if (UniversalPlatform.isWeb) {
        // Skip silent sign-in on web due to GIS compatibility issues
        authSuccess = false; // Force user to manually sign in
      } else {
